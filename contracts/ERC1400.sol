...
import "./extensions/tokenExtensions/ERC1400TokensChecker.sol";
//make sure the non interface extensions are in userExtensions folder
import "./extensions/userExtensions/ERC1400TokensSender.sol";
import "./extensions/userExtensions/ERC1400TokensRecipient.sol";
import "./tools/DomainAware.sol";

import "./CertificateSigner.sol";

/**
 * @title ERC1400
 * @dev ERC1400 logic
 */
contract ERC1400 is IERC20, IERC1400, ERC1820Client,ERC1820Implementer,Ownable, MinterRole, DomainAware, CertificateSigner{
    ...
    //Indicate whether ERC1820 registry can still be set
    ...
    */
    constructor(
        ...
        address[] memory certifiedSigners
    )
        public
    {
        _name = name;
        ...
        _addCertifiedSigners(certifiedSigners);
        ...
        _setInterface(ERC1400_INTERFACE_NAME); // For migration
        _setInterface(ERC20_INTERFACE_NAME); // For migration
    ...
    /************************************************************************************************/
    /****************************** ER1820 Implementation ****************************/
    /************************************************************************************************/
   
    /**
    * @dev Sets the record in the registry
    * @return True if successful
    */
    function setInterface() external onlyOwner returns (bool){
    ...
    /**
    * @dev Set the ERC1820 registry 
    * @param _address Address of ERC1820 registry for that contract
    */
    function setERC1820(address _address) external onlyOwner returns(bool){
        //Checks that ERC1820 has not already been set
        require(_registerIsSettable, "Register has already been set");

        //Set ERC1820 registry of the asset
        ERC1820REGISTRY = ERC1820Registry(_address);
    ...
    //We need to think about the security issues here as transferring the asset to another address would not transfer ownership
    //Potentially remove ERC20 interfact part and find another way to be compatible to deposit in a hardware wallet
    ...
    */
    ...
    hasValidCertificate(address(this), to)
    returns (bool) {
    ...
    */
    function authorizeOperator(address operator) external override {
        ...
        _authorizedOperatorByPartition[msg.sender][_defaultPartitions[0]][operator] = true;
        emit AuthorizedOperatorByPartition(_defaultPartitions[0], operator, msg.sender);
    }
    /**
    ...
    */
    function revokeOperator(address operator) external override {
        ...
        _authorizedOperatorByPartition[msg.sender][_defaultPartitions[0]][operator] = false;
        emit RevokedOperatorByPartition(_defaultPartitions[0], operator, msg.sender);
    }
    /**
    ...
    */
    ...
    // We need to think about the lock up period of the funds - an idea would be to change the code to have the default partition 0 to represent the locked up funds that then get moved into default 1 which is the one where transfers can occur
    function issue(address tokenHolder, uint256 value, bytes calldata data)
        external
        override
        onlyMinter
        hasValidCertificate(address(this), tokenHolder)
        isIssuableToken
    {
    ...
    */
    function renounceIssuance() internal onlyOwner {
        _isIssuable = false;
    }
    /************************************************************************************************/
    ...
    */
    ...
    function _addCertifiedSigners(address[] memory signers) internal {
        for (uint i = 0; i<signers.length; i++){
            _addCertificateSigner(address(this), signers[i]);
        }
    }
    /**
    * @dev Set list of token partition controllers.
