...
contract DVP is Ownable, ERC1820Client, IERC1400TokensRecipient, ERC1820Implementer {
  ...
  enum SaleType{Initial, Secondary}
   ...
   * @param saleType indicates whether we are in the initial sale or not
   */
  ...
  //Indicate whether contract is still registerable in ERC1820 registry
  bool internal _registerIsSettable;
  ...
  /**
   * Set record in ERC1820 registry - can only be done time
   * @return 'true' if the the contract has been successfully registered
   */
  function registerContract() external onlyOwner returns(bool){
    require(_registerIsSettable, "Contract has already been recorded in ERC1820 registry");
    setInterfaceImplementation(ERC1400_TOKENS_RECIPIENT, address(this));
    _registerIsSettable = false;
    return true;
  }

  /**************************************************************************************************************/
  /****************************************** ERC1400 specific functionality ****************************************************/
  /**************************************************************************************************************/
   ...
   */
   // inputData needs to additionally include the KYC documentation such that it can be verified by certificate signer 
   //- might even need to do this verification again here
  ...
  function approveTrade(uint256 index, bool approved) external {
    ...
    if((trade.executer == address(0) || trade.executer == msg.sender) && _tradeisAccepted(index) && _tradeisApproved(index)) {
      _executeTrade(index);
    }
  }
  ...
  /**************************************************************************************************************/
  /*********************************** EXTERNAL: Sale Date Functionality ****************************************/
  /**************************************************************************************************************/
  /**
   * @dev Compute the offer (that has been accepted) of a property.
   *
   * This function can only be called by anyone.
   *
   * @param property of where the property partially fungible token is stored.
   */
  //This should come from off-chain, i.e. javascript dictionary that stores information in webapp
  function _computePropertyOffer(address property) internal returns(uint){
    uint offer = 0;
    for (uint256 i; i < _index; i++) {
      Trade storage trade = _trades[i];
      (,,,,,bool approved1) = (abi.decode(trade.tokenData1, (address, uint256, bytes32, Standard, bool, bool)));
      (address tokenAddress2,uint amount,,,,bool approved2) = (abi.decode(trade.tokenData2, (address, uint256, bytes32, Standard, bool, bool)));

      if (tokenAddress2 == property && approved1 == true && trade.saleType == SaleType.Initial) {
        offer = offer.add(amount);
      } 
    }
    return offer;
  }
  ...
  /**
   * @dev Enable Trade Execution (only executor can do).
   *
   * This function can only be called by the executer specified at trade creation, for each trade.
   *
   * @param data containing all information needed for this function 
   */

  function enableExecution(bytes memory data) external onlyTokenController(msg.sender){

    (address propertyAddr,address ownerAddr, uint256 price, uint256 threshold, address ERC20address, bytes32 tokenId1, bytes32 tokenId2) = abi.decode(data, (address,address, uint256, uint256, address, bytes32, bytes32));

    uint256 supply = ERC1400(propertyAddr).totalSupply();
    uint256 sold = _computePropertyOffer(propertyAddr);

    uint256 totalValue = supply * price;
    uint256 soldValue =  sold * price;
    uint256 saleDate = 0;
    ...
    for (uint i; i < _index; i++) { 
        ...
        if (tokenAddress2 == propertyAddr && approved1 == true) {
          require(trade.executer == msg.sender, "Executer has to be the sender");
          saleDate = trade.expirationDate;
          break;
        }
        
    }
    require(block.timestamp == saleDate, "Can only be called on sale date");
    //Assign Property State

    if (soldValue == totalValue) { //Scenartio Fully sold
      _makeSale(propertyAddr,ownerAddr);
    } else if (soldValue.div(totalValue).mul(100) >= threshold) { //Scenario XTerra Buy in
      _setPartiallySold(propertyAddr, totalValue.sub(soldValue), supply.sub(sold), ERC20address, tokenId1,tokenId2);
    } //otherwise Scenario no sale => buyers can then cancel their trades therefore no action
 ...
 /**************************************************************************************************************/
  ...
  /**************************************************************************************************************/
  /*********************************** Internal: Sale Date Functionality ****************************************/
  /**************************************************************************************************************/
  /**
  * @dev Accepts all property trades on behalf of 
  * This function can only be called by the executer specified at trade creation, for each trade.
  * @param propertyAddr of where the property partially fungible token is stored.
  * @param seller address of the property owner
  */
  function _makeSale(address propertyAddr, address seller) internal
  {
    ...
    for (uint i;i < _index; i++) {
      ...
      if (tokenAddress2 == propertyAddr && approved1 == true && trade.saleType == SaleType.Initial) {
        _acceptTrade(i, seller, 0, 0);
        this.approveTrade(i, true);
      }
    }
  }
  ...
  /**
  * @dev Requests trade for the remaining balance that has not been purchased
  * @param propertyAddr of where the property partially fungible token is stored.
  * @param value Stablecoin value to be purchased
  * @param ERC1400value value of token to be received
  * @param ERC20address address of Stablecoin
  * @param tokenId1 partition of Stablecoin
  * @param tokenId2 partition of ERC1400
  */
  function _setPartiallySold(address propertyAddr, uint256 value, uint256 ERC1400value, address ERC20address, bytes32 tokenId1, bytes32 tokenId2) internal {

      TradeRequestInput memory request = TradeRequestInput({
      holder1: propertyAddr,
      holder2: address(0),
      executer: msg.sender,
      expirationDate: block.timestamp + 60*60*24,
      tokenAddress1: ERC20address,
      tokenValue1: value,
      tokenId1:tokenId1,
      tokenStandard1: Standard.ERC20,
      tokenAddress2: propertyAddr,
      tokenValue2: ERC1400value,
      tokenId2:tokenId2,
      tokenStandard2: Standard.ERC1400,
      tradeType: TradeType.Escrow,
      saleType: SaleType.Initial
    });
    
    this.requestTrade(request);
    this.approveTrade(_index, true); //given that the executor is calling it, it also calls the _execute function
  }
